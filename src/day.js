...
`;
        ...
        for (var i in dayobj.time) {
            ...
            var draftstr = "";
            if (timeslice.type == "work") {
                var draftfilename = path.draftrepopath + year + "/" + timeslice.begin + ".md";
    ...
    makedayplan: function (dayobj) {
        ...
        var dayplanstr = "# " + date.Format("yyyy.MM.dd.") + "\n日计划\n\n"
            + "根据[ego模型时间接口](https://gitee.com/hyg/blog/blob/master/timeflow.md)，六月份安排休整和总结。今天绑定模版" + dayobj.mode + "(" + dayobj.plan + ")。\n\n"
    ...
    gitop: async function (commitmsg) {
        await util.gitstep("D:\\huangyg\\git\\blog", commitmsg, "alls", 'master');
        await util.gitstep("D:\\huangyg\\git\\draft", commitmsg, "gitee", 'master');
        await util.gitstep("D:\\huangyg\\git\\ego", commitmsg, "alls", 'vat');
        await util.gitstep("D:\\huangyg\\git\\js.sample", commitmsg, "alls", 'master');
    }
}